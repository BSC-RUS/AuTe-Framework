...
/** Created by smakarov 23.03.2018 11:50 */
@Component
  ...
  public List<Attachment> extract(File resultDirectory, StepResult result) {
    ...
    if (relativePath != null) {
      return Collections.singletonList(
          new Attachment().withTitle(FILE_NAME).withSource(relativePath).withType(TEXT_PLAIN));
    }
    return null;
  }
        ...
        .map(
            request ->
                "Service name:"
                    ...
                    + "\nSaved variables:\n"
                    + this.getSavedVariables(request.getScenarioVariablesFromServiceRequest()))
        .collect(Collectors.joining("\n\n"));
  }

  private String getSavedVariables(List<ScenarioVariableFromServiceRequest> variables) {
    return variables
        .stream()
        .map(
            var ->
                "Name:\n"
                    + var.getScenarioVariableName()
                    + "\nMatching type:\n"
                    + String.valueOf(var.getMatchingType())
                    + "\nExpression:\n"
                    + var.getExpression())
        .collect(Collectors.joining("\n\n"));
  }
