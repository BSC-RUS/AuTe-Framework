...
export class ProjectDetailComponent implements OnInit, AfterContentChecked {
  ...
  /**
   * Функция обновления чекбокса, что все сценарии выделены
   * @description Если список сценариев пуст, то флаг устанавливается false.
   * Отфильтровывает сценарии по необходимой группе,
   * проверяет на наличие (> 0) элементов в отфильтрованном списке и на отсутствие невыбранных элементов.
   **/
  updateSelectAllFlag() {
    if (this.scenarioList) {
      const list: Scenario[] = this.scenarioList.filter(s => !this.filter || s.scenarioGroup === this.filter.scenarioGroup);
      this.selectAllFlag = list.length > 0 && !list.some(s => !s._selected);
    } else {
