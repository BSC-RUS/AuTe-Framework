...
import { ScenarioVariableFromServiceRequest } from '../model/scenario-variable-from-service-request';
...
export class StepItemComponent implements OnInit, DoCheck {
    ...
    addStepBefore() {
        const addAfterIndex = this.stepList.indexOf(this.step);
        if (addAfterIndex > -1) {
    ...
    addSavedVariableFromMockRequest(expectedServiceRequest: ExpectedServiceRequest) {
        if (!expectedServiceRequest.scenarioVariablesFromServiceRequest) {
            expectedServiceRequest.scenarioVariablesFromServiceRequest = [];
        }
        expectedServiceRequest.scenarioVariablesFromServiceRequest.push(new ScenarioVariableFromServiceRequest());
    }

    removeSavedVariableFromMockRequest(
        expectedServiceRequest: ExpectedServiceRequest,
        scenarioVariableFromServiceRequest: ScenarioVariableFromServiceRequest
    ) {
        if (confirm('Confirm: remove saved variable from service request')) {
            const indexToRemove = expectedServiceRequest.scenarioVariablesFromServiceRequest.indexOf(scenarioVariableFromServiceRequest);
            if (indexToRemove > -1) {
                expectedServiceRequest.scenarioVariablesFromServiceRequest.splice(indexToRemove, 1);
            }
        }
    }
